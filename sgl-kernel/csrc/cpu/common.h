...
// Like parallel_2d but with explicit nth_t × nth_k decomposition.
// Caller is responsible for choosing nth_t and nth_k.
template <typename func_t>
inline void parallel_2d_tiled(int m, int n, int nth_t, int nth_k, const func_t& f) {
  const int nth = nth_t * nth_k;
#if defined(_OPENMP)
#pragma omp parallel num_threads(nth)
  {
    ...
    int thread_block_m = div_up(m, nth_t);
    int thread_block_n = div_up(n, nth_k);
