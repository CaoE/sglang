"""XpuCudaGraphBackend — Ascend XPU full-graph capture (torch.xpu.XPUGraph).

Mirrors FullCudaGraphBackend with XPU-specific differences:
  - Captures via torch.xpu.graph(xpu_graph=...) into torch.xpu.XPUGraph.
  - No set_graph_pool_id (XPU does not use pynccl allocator).
"""
...
class XpuCudaGraphBackend(BaseCudaGraphBackend):
    ...
    def capture_one(
        ...
        with graph_ctx(xpu_graph=graph, pool=self._pool, stream=self._capture_stream):
            out = forward_fn()
